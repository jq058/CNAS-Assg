...
The current application image and Compose profile apply these controls:

- Apache/PHP runs as www-data on unprivileged port 8080.
- The image copies only runtime PHP, asset, and configuration files.
- Build-only compiler packages are removed after PHP extensions are installed.
- Application files are read-only and internal PHP modules are denied by Apache.
- Compose drops all Linux capabilities and enables no-new-privileges for the web and Redis containers.
...
- Health checks distinguish process liveness from MySQL/Redis-dependent readiness.
- Secrets must come from the ignored .env file; .env.example contains placeholders only.
